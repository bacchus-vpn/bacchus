...
type wire struct {
	...
	// row for the connecting account's (trust, plan) pair — a number the
	// coordinator looked up in a document it verified against the policy root, not
	// one it authored and not one any client or node asserted.
	//
	// Distinct from SpeedCap above, which is a NODE's own declared aggregate cap on
	// register: that one is what an operator is willing to carry in total, this one
	// is what one user's tier entitles one session to. They compose rather than
	// override — the node's aggregate limiter still paces every byte, so a
	// coordinator that stamps a large number here cannot raise what the node
	// already refuses to exceed (ADR-0040).
	//
	// Zero/absent means no cap, which is both the pre-#58 behaviour and what an
	// unpoliced coordinator sends: capacity.NewLimiter(0) is nil and nil-inert, so
	// the exit needs no branch for it.
	//
	// Sent only on assigns that reach an EXIT — a direct connect or a relay-mode
	// TURN fallback. A peer-relay assign goes to the RELAY (it carries ExitAddr),
