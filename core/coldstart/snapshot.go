...
type Entry struct {
	...
	// an INTERMEDIATE relay hop in a multi-hop chain (old #124, ADR-0038 §9 item 3).
	...
	// (old #31) — a NAT-punched hole, not a forward-dial target — whereas Ingress is a
	...
	// address seam ADR-0037 deferred and the field the whole relay-chaining epic (old #76)
	...
	// omitempty keeps a snapshot predating old #124 byte-identical on the wire (backward-compat).
	Ingress string `json:"ingress,omitempty"`

	// Operator is the coordinator-known operator / vouch-subtree tag, used for
	// operator-diversity hop selection (old #124, ADR-0038 §6): §4 avoids routing two
...
// client-assembled relay chain (old #76): a relay that advertises a forwarding
// Ingress. It is the directory-side field-level gate the old #124 acceptance names —
...
// snapshots can be freely re-shared by a mesh-walk courier (old #31) or the
// coordinator pool (old #6) without leaking anyone's credential.
...
// courier's proof-of-prior-contact check (old #31, design §4.3): a recovering
...
// snapshot order. Mesh-walk recovery (old #31) uses it two ways: "coordinator"
