...
type Config struct {
	...
	// public key. This is a distinct, stronger guarantee than InsecureSkipVerify:
	// InsecureSkipVerify only disables chain-of-trust and hostname checks, while
	// the CertificateVerify signature proves the peer holds the leaf's private
	// key at all, independent of who issued the certificate.
	//
	// Skipping it is only safe when the caller has an out-of-band reason to trust
	// the connection regardless of what key signed the handshake transcript (for
	// example, an application-layer mutual authentication step carried inside the
	// TLS session). It must never be set as a blanket TLS downgrade.
	//
	// The check still runs; only a verification failure is tolerated instead of
	// aborting the handshake, so a correct signature is never silently ignored.
...
func (c *Config) Clone() *Config {
	...
	return &Config{
		...
		InsecureSkipCertVerifySignature:     c.InsecureSkipCertVerifySignature,
