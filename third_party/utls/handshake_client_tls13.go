...
func (hs *clientHandshakeStateTLS13) readServerCertificate() error {
	...
	// [BACCHUS] Upstream always aborts here on a bad signature. When the caller has
	// opted in via InsecureSkipCertVerifySignature (see common.go), a failure is
	// tolerated instead: the check still runs, so a *correct* signature is never
	// silently ignored, but an incorrect one no longer aborts the handshake.
	if err := verifyHandshakeSignature(sigType, c.peerCertificates[0].PublicKey,
		sigHash, signed, certVerify.signature); err != nil && !c.config.InsecureSkipCertVerifySignature {
