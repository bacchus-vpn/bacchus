...
#### 4.2.1 What is built, as of ADR-0061 — and what is not

Layer 2 and layer 3 are built for the **desktop client** (`#193`). It takes a
`bacchus1:` invite in its config (`cmd/coldstart-issue` mints it, layer 2), and at
each connect it fetches a fresh signed snapshot from the coordinator that invite
names, verifies it, caches the signed bytes, and reads its coordinator pool and the
account service's address out of it (layer 3). Before that, every address it dialled
was a constant in a JSON file with no way to correct it — the mechanism had existed
since `#18` and no client had adopted it.

Three things are deliberately not built:

- **Layer 1 has no implementation.** There is no distribution site handing out a
  current bundle at download time, and there is nothing baked into a public build.
- **No invite is generated by the app**, so the "existing user generates one" half of
  layer 2 is an operator running `coldstart-issue` today. The invite is
  per-recipient and is never placed in an installer or a config template — a
  coordinator's secrets file has no trust system under it, so a shared one is a
  credential for the whole network held by everyone who downloaded the artifact.
- **The client does not walk the mesh** (§4.3). It caches the signed snapshot, which
  is the proof of prior contact a walk has to present, but does not use it: a walk
  asks a peer running a courier listener on a separate address the snapshot does not
  carry.
