...
not anticipate (ADR-0044), and item 5 twice, its config surface having re-opened under it
when the desktop client of record changed and closed again with #93.
...
| 5 | Hop-count knob + config surface | #28, then #93 | **Shipped, re-opened by a client change, and closed again.** Landed for core, `cmd/node`, and the walk client's ADR-0036 "Relay hops" control (`Config.RelayHops` / `-relay-hops`, default 1, refused above `RelayHopsMax`), which also owns the directory file + key fields and surfaces `[relay] chain not built:` distinctly from a generic connection error. #35 then made Fyne the desktop client of record, and `clients/fyne` had **no** `RelayHops`/`RelayDirectory` settings surface — the knob absent from the client replacing the one that had it. That remainder was tracked by nothing until **#93**, which built the control in `clients/fyne` (hop count clamped to `[1, RelayHopsMax]`, directory path and key required together, both refused at Save rather than at connect) along with the other five `core.Config` fields the two dialogs disagreed on, and deleted the walk client's inert `ExitID` pin. The `[relay] chain not built:` distinction needed no work: `clients/fyne` had mirrored the walk client's constant since its `DetailFor` was written, against the day a control existed to produce it. #93's own finding is the durable part, and it is recorded in ADR-0039 rather than here: the eight-point parity bar is entirely about **enforcement**, so it could be met in full while the client of record could not configure half of what core supports. |
