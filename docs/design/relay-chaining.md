...
- Status: **design spike (issue #76; this document + [ADR-0038](../adr/0038-configurable-multi-hop-relay-chaining.md); implementation follows in child issues)**
- Date: 2026-07-11
...
> This is a **design spike**, not an implementation. Like the cold-start spike
> (issue #18a → ADR-0012, which preceded the #18b build), it fixes the shape of the
> mechanism and its threat-model defences, and hands the build to child issues (§9).
> The one thing it builds now is a dependency-free feasibility probe
> ([`cmd/relaychain-probe`](../../cmd/relaychain-probe/README.md)) that demonstrates
> the central cryptographic claim — nested Noise_NK layers peel one-per-hop and no
> intermediate hop learns the endpoints — before any production wiring exists.
...
**What onion chaining defends, precisely.** For a chain of `n ≥ 2` relays:
...
- **Hard upper bound `RelayHopsMax`** (proposed `4`; §6). Values above the cap are
  clamped with a logged notice; the client never builds an unbounded chain.
...
- **Coordinator still sees client↔R₁.** First-hop rendezvous is coordinator-brokered,
  as today; the coordinator learns the client contacted *some* relay, never which exit
  or which downstream hops. This is unchanged from single-hop and is the residual the
  pool + vouch model already own.
