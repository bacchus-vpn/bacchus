...
// ReconcileLaunchOnBoot aligns the OS-level autostart registration with
// cfg.LaunchOnBoot, meant to run once at every startup before anything else
// touches it (issue #170). The two directions are deliberately NOT
// symmetric:
//
//   - cfg.LaunchOnBoot == false: always ensures the OS entry is absent,
//     unconditionally (SetLaunchOnBoot's own idempotent no-op when it's
//     already gone). There is nothing to "fight" in this direction.
//   - cfg.LaunchOnBoot == true: refreshes the entry only if
//     LaunchOnBootActive reports it is still present — self-healing a stale
//     path after the binary moved, via SetLaunchOnBoot's overwrite. If it is
//     gone, that is treated as the user having turned it off outside the
//     app (a hand-deleted .desktop file on Linux; a hand-removed Run value
//     on Windows — see each platform's LaunchOnBootActive doc for what this
//     does and does not detect there), and the returned Config has
//     LaunchOnBoot corrected to false rather than recreating the entry. A
//     tool whose users depend on it to mean what it says must not resurrect
//     something they explicitly turned off through a door of their own
//     choosing — the same principle #169 applies to the kill-switch
//     checkbox, applied here to a lower-stakes but still user-facing claim.
//
// Returns the (possibly corrected) Config so the caller can persist it and
// keep the Settings window's checkbox truthful — compare LaunchOnBoot
// between the return value and the input to know whether a save is needed —
// and any error LaunchOnBootActive/SetLaunchOnBoot returned.
func ReconcileLaunchOnBoot(cfg Config) (Config, error) {
	if !cfg.LaunchOnBoot {
		return cfg, SetLaunchOnBoot(false)
	}
	...
	if active {
		return cfg, SetLaunchOnBoot(true)
	}
	cfg.LaunchOnBoot = false
	return cfg, nil
}
